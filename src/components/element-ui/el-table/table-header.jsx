...
export default {
      ...
      <table class="el-table__header" cellspacing="0" cellpadding="0" border="0">
        ...
        <thead class={[{ 'is-group': isGroup, 'has-gutter': this.hasGutter }]}>
          {this._l(columnRows, (columns, rowIndex) => (
            <tr ref={'sortabeRef_' + rowIndex} style={this.getHeaderRowStyle(rowIndex)} class={this.getHeaderRowClass(rowIndex)}>
              {columns.map((column, cellIndex) => (
                ...
                >
                  <div class={['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName]}>
                    {column.renderHeader ? column.renderHeader.call(this._renderProxy, h, { column, $index: cellIndex, store: this.store, _self: this.table.$vnode.context }) : column.label}
                    {column.sortable ? (
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      const _this = this
      new Sortable(this.$refs.sortabeRef_0, {
        animation: 150,
        draggable: 'th',
  ...
  methods: {
    ...
    onSortableEnd(e) {
      ...
      if (target.level !== 1 || target.fixed) {
        e.preventDefault()
        return
      }
      ...
      this.table.$emit(
        'header-cell-dragend',
        _columns.map(x => ((x.realWidth = x.width), { label: x.label, width: x.width })),
      )
      e.preventDefault()
    },
